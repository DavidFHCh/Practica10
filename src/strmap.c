// Implementación de una hashmap para strings encontrada en pokristensson.com/code/strmap, utilizada bajo la licencia abierta GNU probablemente.
// No tengo tiempo para implementar una por mi mismo. Gracias, software libre.
#include "strmap.h"
...
int sm_get(const StrMap *map, const char *key, char *out_buf, unsigned int n_out_buffer)
{
	unsigned int index;
	Bucket *bucket;
	Pair *pair;
	...
	if(pair == NULL)
		return 0;
	if(out_buf == NULL && n_out_buffer == 0)
		return strlen(pair->value) + 1;
	if(out_buf == NULL)
		return 0;
	if(strlen(pair->value ) >= n_out_buffer)
		return 0;
...
int sm_exists(const StrMap *map, const char *key)
{
	...
	if(map == NULL)
		return 0;
	if(key == NULL)
		return 0;index = hash(key) % map->count;
	bucket = &(map->buckets[index]);
	pair = get_pair(bucket,key);
	if(pair == NULL)
		return 0;
